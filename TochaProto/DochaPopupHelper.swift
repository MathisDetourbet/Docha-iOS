...
public class DochaPopupHelper {
    ...
    func showLoadingPopup(title: String?="") -> UIAlertController? {
        ...
        self.alertController?.view.translatesAutoresizingMaskIntoConstraints = false

        let heightConstraint = NSLayoutConstraint(item: self.alertController!.view, attribute: .Height, relatedBy: .Equal, toItem: nil, attribute: .NotAnAttribute, multiplier: 1, constant: 130)
        ...
        let size = CGSize(width: widthConstraint.constant, height: heightConstraint.constant)
        let indicatorView = NVActivityIndicatorView(frame: CGRectMake(0.0, 0.0, 50.0, 50.0), type: NVActivityIndicatorType.BallClipRotate, color: UIColor.blueDochaColor(), padding: 0.0)
        indicatorView.center = CGPoint(x: size.width / 2, y: size.height-(indicatorView.frame.height/2 + 5.0))
